import {
  getAllRepliesComments,
  getCountRepliesComments,
} from "@/services/comment.service";
import { CommentModel } from "@/types/comment";
import { useCallback, useEffect, useState } from "react";
import {
  registerRepliesAppender,
  unregisterRepliesAppender,
} from "@/hooks/repliesRegistry";
...
export default function useReplies(commentId: number) {
  ...
  const appendReply = useCallback((reply: CommentModel) => {
    ...
    handleGetAllRepliesComment(commentId);
  }, []);

  // Hàm xóa reply khỏi state
